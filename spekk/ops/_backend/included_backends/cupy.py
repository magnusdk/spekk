...
import cupy as cp
from array_api_compat.cupy import *
from array_api_compat.cupy import _info
...
def getitem_along_axis(x, axis: int, i: int):
    ...
    try:
        ...
        try:
            return cp.array(x).__getitem__(slice_)
        except Exception:
            raise ValueError(
...
def _python_vmap(f, in_axes=None):
    ...
    def wrapped(*args):
        ...
        if in_axes is None:
            in_axes = [0] * len(args)
        sizes = {cp.shape(x)[ax] for x, ax in zip(args, in_axes) if ax is not None}
        if len(sizes) != 1:
            raise ValueError(
...
def scan(fn, init, xs):
    ...
    for leaf_idx in range(num_leaves):
        ...
        dynamic_result.append(cp.stack(leaf_values, axis=0))
...
def flatten(x: cp.ndarray) -> cp.ndarray:
    return x.flatten()
